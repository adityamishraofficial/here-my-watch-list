...
class MovieDetails extends Component<any> {
    ...
    onLoadMovie(){
        ...
        axios.get(API_URLS.MOVIE_BY_ID + that.props.params.id, {
            params: API_PARAMS
          })
    ...
    onLoadRecommendMovie() {
        const that = this;
        this.setState({loading: this.state.loading + 1})
        const path = API_URLS.RECOMMEND_MOVIES.replace('{ID}', that.props.params.id)
        axios.get(path, {
            params: API_PARAMS
          })
          .then(function (response) {
    ...
    //    console.log('movieDetails', movieDetails.spoken_languages)
